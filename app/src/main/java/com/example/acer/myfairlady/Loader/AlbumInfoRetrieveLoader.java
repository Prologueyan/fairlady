...
public class AlbumInfoRetrieveLoader extends AsyncTaskLoader<List<AlbumInfo>> {
     ...
     * 要从MediaStore检索的列
     */
    private final String[] mProjection = new String[]{MediaStore.Audio.Albums.ALBUM,
    ...
    public List<AlbumInfo> loadInBackground() {
        ...
        if (cursor != null) {
            int index_album_name = cursor.getColumnIndex(MediaStore.Audio.Albums.ALBUM);
            int index_album_id = cursor.getColumnIndex(MediaStore.Audio.Albums._ID);
            int index_num_of_songs = cursor
                    .getColumnIndex(MediaStore.Audio.Albums.NUMBER_OF_SONGS);
            int index_art_work = cursor.getColumnIndex(MediaStore.Audio.Albums.ALBUM_ART);
